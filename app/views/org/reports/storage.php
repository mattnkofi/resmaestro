...
<body class="bg-maestro-bg text-white font-poppins" x-data="{}">
    ...
    <div class="ml-64 p-8 bg-maestro-bg min-h-screen text-white">
        ...
        $storage_stats = [
            ...
            'used_pct' => 73,
            'file_types' => [
                ['type' => 'PDF', 'size_gb' => 4.5, 'color' => 'red'],
                ['type' => 'DOCX', 'size_gb' => 1.8, 'color' => 'blue'],
                ['type' => 'XLSX', 'size_gb' => 0.5, 'color' => 'green'],
                ['type' => 'Images', 'size_gb' => 0.5, 'color' => 'yellow'],
            ]
        ];
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-6 mb-8">
            ...
            <!-- Card 5 (NEW): Storage Usage (From snippet) -->
            <div class="bg-green-950/50 p-5 rounded-xl border border-green-800 shadow-lg flex flex-col justify-between md:col-span-1 col-span-2">
                <p class="text-sm text-gray-400 uppercase tracking-wider flex items-center gap-2">
                    <i class="fa-solid fa-database"></i> Total Storage
                </p>
                <p class="text-3xl font-bold text-blue-400 mt-1"><?= $storage_stats['used_gb'] ?>GB</p>
                <p class="text-xs text-gray-500">of <?= $storage_stats['total_gb'] ?>GB used</p>
                <div class="w-full bg-green-900 rounded-full h-3 mt-2">
                    <div class="bg-blue-500 h-3 rounded-full" style="width:<?= $storage_stats['used_pct'] ?>%"></div>
                </div>
            </div>
        </div>
        ...
        <!-- Main Content Grid: Detailed Reports (2 rows) -->
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
            
            <!-- Left/Center Column (3/5 width): Breakdown Table -->
            ...
            <!-- Right Column (2/5 width): Document Insights -->
        ...
        </div> <!-- End Document Breakdown Grid -->
        
        <!-- NEW: Storage Details and Trend -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
            
            <!-- Left Column: File Type Breakdown Table -->
            <div class="lg:col-span-1 space-y-4">
                <h2 class="text-xl font-semibold text-green-300 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-hdd text-lg"></i> Storage Breakdown by File Type
                </h2>
                ...
                <div class="overflow-x-auto rounded-xl border border-green-800 shadow-2xl shadow-green-900/10">
                    <table class="w-full text-left">
                        <thead class="bg-green-900/40 text-gray-200 uppercase text-sm tracking-wider">
                            <tr>
                                <th class="p-4 border-b border-green-800">File Type</th>
                                <th class="p-4 border-b border-green-800 text-center">Volume (GB)</th>
                                <th class="p-4 border-b border-green-800 text-center">Percentage</th>
                            </tr>
                        </thead>
                        <tbody class="bg-[#0f1511] text-gray-300">
                            <?php 
                            $color_map = ['red' => 'text-red-400', 'blue' => 'text-blue-400', 'green' => 'text-green-400', 'yellow' => 'text-yellow-400'];
                            foreach($storage_stats['file_types'] as $file_type): 
                                $pct = round(($file_type['size_gb'] / $storage_stats['used_gb']) * 100);
                            ?>
                            <tr class="border-b border-green-800 hover:bg-green-700/10 transition">
                                <td class="p-4 font-medium <?= $color_map[$file_type['color']] ?? 'text-gray-300' ?>"><?= $file_type['type'] ?></td>
                                <td class="p-4 text-center font-bold text-gray-300"><?= number_format($file_type['size_gb'], 1) ?></td>
                                <td class="p-4 text-center text-gray-300"><?= $pct ?>%</td>
                            </tr>
                            <?php endforeach; ?>
                            <tr class="font-bold bg-green-900/50">
                                <td class="p-4">Total Used</td>
                                <td class="p-4 text-center"><?= number_format($storage_stats['used_gb'], 1) ?></td>
                                <td class="p-4 text-center"><?= $storage_stats['used_pct'] ?>%</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <!-- Right Column: Storage Trend Chart -->
            <div class="lg:col-span-1">
                <div class="bg-green-950/50 p-6 rounded-xl border border-green-800 shadow-2xl h-full space-y-6">
                    <h2 class="text-xl font-bold text-green-300 flex items-center gap-2 border-b border-green-800/50 pb-2">
                        <i class="fa-solid fa-chart-area text-lg"></i> Storage Usage Trend
                    </h2>
                    <!-- Chart Placeholder -->
                    <div class="h-64 flex items-center justify-center text-gray-500 border border-dashed border-green-800/50 rounded-lg">
                        Storage Growth Chart Placeholder (GB over time)
                    </div>
                    <a href="<?=BASE_URL?>/org/reports/storage" class="mt-4 block text-sm text-green-400 hover:text-green-300">Manage Storage Quotas &rarr;</a>
                </div>
            </div>
        </div>
        <!-- END Storage Details and Trend -->
    </div>
</body>
</html>