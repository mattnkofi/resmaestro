...
<head>
    ...
    <title>Review History - Maestro UI</title>
...
<body class="bg-maestro-bg text-white font-poppins" x-data="{}">
    ...
    // For "Review History" page:
    $current_uri = $_SERVER['REQUEST_URI'] ?? '/org/review/history'; 
    ...
    <div class="ml-64 p-8 bg-maestro-bg min-h-screen text-white">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-8">
            <input type="text" placeholder="Search by document, reviewer, or date..." 
        ...
        <!-- Review History List (Dynamic Cards) -->
        <div class="space-y-4">
            <?php
            // Mock data for the review history
            $review_history = [
                ['title' => 'Staff Guidelines V1.2', 'reviewer' => 'Jane Doe', 'status' => 'Approved', 'date' => 'Oct 10, 2025', 'accent' => 'green'],
                ['title' => 'Q1 Sales Plan Draft', 'reviewer' => 'Admin User', 'status' => 'Rejected', 'date' => 'Oct 5, 2025', 'accent' => 'red'],
                ['title' => 'New Expense Policy', 'reviewer' => 'John Smith', 'status' => 'Approved', 'date' => 'Sep 28, 2025', 'accent' => 'green'],
                ['title' => 'Software Purchase Request', 'reviewer' => 'Ellaine Cordero', 'status' => 'Rejected', 'date' => 'Sep 15, 2025', 'accent' => 'red'],
            ];

            foreach($review_history as $doc): 
                $status_color = $doc['status'] == 'Approved' ? 'text-green-400' : 'text-red-400';
                $border_color = $doc['status'] == 'Approved' ? 'border-green-500' : 'border-red-500';
            ?>
            <div class="bg-green-950/50 p-5 rounded-xl border-l-4 <?= $border_color ?> flex flex-col md:flex-row justify-between items-start md:items-center shadow-lg hover:bg-green-900/40 transition">
                <div class="flex flex-col mb-2 md:mb-0">
                    <span class="text-lg font-semibold text-green-200"><i class="fa-solid fa-file-alt mr-2"></i> <?= $doc['title'] ?></span>
                    <span class="text-sm text-gray-400">Reviewed by: <?= $doc['reviewer'] ?></span>
                </div>
                <div class="flex items-center space-x-6">
                    <div class="text-right hidden sm:block">
                        <span class="block text-xs text-gray-300 uppercase tracking-wider">Status:</span>
                        <span class="block text-sm <?= $status_color ?> font-medium"><?= $doc['status'] ?> on <?= $doc['date'] ?></span>
                    </div>
