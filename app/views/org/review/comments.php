...
<head>
    ...
    <title>Review Comments - Maestro UI</title>
...
<body class="bg-maestro-bg text-white font-poppins" x-data="{}">
    ...
    <aside class="fixed top-0 left-0 h-full w-64 bg-[#0b0f0c] border-r border-green-900 text-white shadow-2xl flex flex-col transition-all duration-300 z-10">
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-3 space-y-4">
            ...
            <!-- Review & Workflow Dropdown (This will be open) -->
    ...
    <div class="ml-64 p-8 bg-maestro-bg min-h-screen text-white">
        ...
        <div class="mb-6 p-4 bg-blue-950/20 border border-blue-800 rounded-lg">
            <p class="text-sm font-semibold text-blue-400">Viewing Comments for:</p>
            <p class="text-xl font-bold text-green-200">Q4 Sales Plan Draft</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-4 max-h-[70vh] overflow-y-auto pr-3">
                
                <?php
                // Mock data for the comment thread
                $comments = [
                    ['user' => 'Ellaine Cordero', 'time' => '1 hour ago', 'role' => 'Reviewer', 'text' => 'Please clarify section 2 regarding variable costs before final approval. The figures seem inconsistent with the Q3 report.', 'color' => 'blue'],
                    ['user' => 'Admin User', 'time' => '45 minutes ago', 'role' => 'Author', 'text' => 'Acknowledged. I\'ve updated the figures in the main document. Section 2.1 now reflects the corrected numbers. Ready for re-review.', 'color' => 'green'],
                    ['user' => 'John Smith', 'time' => '20 minutes ago', 'role' => 'Reviewer', 'text' => 'The branding language in the executive summary is slightly off-message. See my suggested edits.', 'color' => 'blue'],
                ];
                
                foreach($comments as $comment): 
                    $user_color = $comment['color'] == 'blue' ? 'text-blue-400' : 'text-green-400';
                    $bg_color = $comment['color'] == 'blue' ? 'bg-blue-900/40' : 'bg-green-900/40';
                ?>
                <div class="<?= $bg_color ?> p-4 rounded-xl border border-gray-700 shadow-lg">
                    <div class="flex items-center justify-between mb-2 border-b border-gray-700 pb-1">
                        <p class="font-bold text-sm <?= $user_color ?>"><?= $comment['user'] ?> 
                            <span class="text-xs text-gray-500 ml-2">(<?= $comment['role'] ?>)</span>
                        </p>
                ...
                <div class="p-8 text-center text-gray-500 bg-blue-950/20 rounded-xl border border-blue-800">
                    <i class="fa-solid fa-comments text-4xl mb-3 text-blue-500"></i>
                    <p class="text-lg">No comments have been left on this document yet.</p>
                </div>
                <?php endif; ?>

            </div>

            <!-- Right Column: Comment Submission Form -->
            <div class="space-y-4">
                <div class="bg-blue-950/50 p-6 rounded-xl border border-blue-800 shadow-2xl">
                    <h2 class="text-xl font-bold text-blue-300 mb-4">Add Your Comment</h2>
                    <form>
                        <textarea class="w-full bg-blue-900 border border-blue-800 p-3 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-white placeholder-gray-500" 
                                  rows="5" placeholder="Type your feedback, question, or reply here..."></textarea>
                        <button type="submit" class="mt-4 w-full bg-blue-700 px-4 py-3 rounded-xl hover:bg-blue-600 font-bold text-lg transition shadow-lg shadow-blue-900/40">
                            <i class="fa-solid fa-paper-plane mr-2"></i> Submit Comment
                        </button>
                    </form>
                </div>
            </div>
