...
<body class="bg-maestro-bg text-white font-poppins" x-data="{}">
    ...
    $doc_id = $doc['id'] ?? null;
    $doc_title = $doc['title'] ?? 'Document Not Found';
    $comments = $comments ?? [];
    ?>
    ...
    <div class="ml-64 p-8 bg-maestro-bg min-h-screen text-white">
        ...
        <div class="mb-6 p-4 bg-blue-950/20 border border-blue-800 rounded-lg">
            ...
            <p class="text-xl font-bold text-green-200"><?= htmlspecialchars($doc_title) ?></p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-4 max-h-[70vh] overflow-y-auto pr-3">
                ...
                foreach($comments as $comment): 
                    // Simple heuristic to distinguish reviewer/author (can be improved with roles/user_id comparison)
                    $user_color = str_contains(strtolower($comment['fname']), 'admin') ? 'text-green-400' : 'text-blue-400';
                    $bg_color = str_contains(strtolower($comment['fname']), 'admin') ? 'bg-green-900/40' : 'bg-blue-900/40';
                    $user_role = str_contains(strtolower($doc_title), 'plan') ? 'Reviewer' : 'Author'; // Mock role
                ?>
                <div class="<?= $bg_color ?> p-4 rounded-xl border border-gray-700 shadow-lg">
                    <div class="flex items-center justify-between mb-2 border-b border-gray-700 pb-1">
                        <p class="font-bold text-sm <?= $user_color ?>"><?= htmlspecialchars("{$comment['fname']} {$comment['lname']}") ?> 
                            <span class="text-xs text-gray-500 ml-2">(<?= $user_role ?>)</span>
                        </p>
                        <p class="text-xs text-gray-500"><?= date('M j, Y H:i', strtotime($comment['created_at'])) ?></p>
                    </div>
            ...
            <div class="space-y-4">
                <div class="bg-blue-950/50 p-6 rounded-xl border border-blue-800 shadow-2xl">
                    ...
                    <form method="POST" action="<?=$BASE_URL?>/org/review/add_comment">
                        <input type="hidden" name="document_id" value="<?= $doc_id ?>">
                        <textarea name="comment_text" class="w-full bg-blue-900 border border-blue-800 p-3 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-white placeholder-gray-500" 
