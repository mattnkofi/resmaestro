...
<body class="bg-maestro-bg text-white font-poppins" x-data="{}">
    ...
    <div class="ml-64 p-8 bg-maestro-bg min-h-screen text-white">
        ...
        <?php
        // Mock data initialization (from previous version)
        $members = [
            ['name' => 'Matt Justine Martin', 'role' => 'Reviewer', 'department' => 'Finance', 'email' => 'martin.m@maestro.com'],
            ['name' => 'Kimberly Nicole De Leon', 'role' => 'Secretary', 'department' => 'HR', 'email' => 'kimberly.c@maestro.com'],
            ['name' => 'Aron Luigee Jordan', 'role' => 'Administrator', 'department' => 'Executive', 'email' => 'jordan.s@maestro.com'],
            ['name' => 'Shirin Chisty Deliverio', 'role' => 'Reviewer', 'department' => 'Legal', 'email' => 'shirincrystal.d@maestro.com'],
        ];
        
        // Stats Calculation
        $total_members = count($members);
        // Using array_filter to count reviewers
        $active_reviewers = count(array_filter($members, fn($m) => $m['role'] === 'Reviewer')); 
        // Using array_column and array_unique to count departments
        $total_departments = count(array_unique(array_column($members, 'department'))); 
        ...
        $member_activity = [
            ...
            ['name' => 'Shirin Chisty Deliverio', 'action' => 'completed 3 reviews', 'time' => 'Yesterday'],
        ];
        ?>

        <!-- NEW: Statistical Summary Row -->
        ...
        <!-- END NEW: Statistical Summary Row -->

        <!-- Main Content Grid: Member List (2/3) + Activity Feed (1/3) -->
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            
            <!-- Left Column: Search & Member List (2/3 width) -->
            <div class="lg:col-span-2 space-y-4">
                
                <!-- Search and Filter (Replaced previous location) -->
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <select class="w-full md:w-1/6 bg-green-900/50 border border-green-800 p-3 rounded-xl text-white">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-green-950/50 p-5 rounded-xl border-l-4 border-green-500 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-lg hover:bg-green-900/40 transition">
                        <div class="flex flex-col mb-2 sm:mb-0">
                            <span class="text-lg font-semibold text-green-200"><?= $member['name'] ?></span>
                            <span class="text-sm text-gray-500"><?= $member['department'] ?> | <?= $member['email'] ?></span>
                        </div>
                        <div class="flex items-center space-x-4">
                            <span class="text-sm font-medium <?= $role_color ?>"><?= $member['role'] ?></span>
            ...
            <!-- Right Column: Recent Member Activity (1/3 width) -->
